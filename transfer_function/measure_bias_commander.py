...
for subset in "", "A", "B":
    ...
    for mc in range(mcstart, mcstop):
        ...
        fn_cl_cmb = "{}/{:04}/clcross_{:04}_{:03}{}_cmb_{:02}fsky.fits".format(
            clcache, mc, mc, freqref, subset, fsky
        )
        if (
            ...
            if not os.path.isfile(fn_cmb) or force:
                fn_in = (
                    ...
                    "quickpol.fits"
                    "".format(simdir, mc, ffpver, freqref, mc, nside_in)
                )
            ...
            if not os.path.isfile(fn_clean) or force:
                if subset == "":
                    fn_in = '/project/projectdirs/planck/data/npipe/commander/' \
                            'npipe6v20/mc_rc1/' \
                            'npipe6v20_commander_cmb_mc_{:04}_005a_2048.fits'.format(mc)
                else:
                    fn_in = '/project/projectdirs/planck/data/npipe/commander/' \
                            'npipe6v20/mc_rc1/' \
                            'npipe6v20_commander_cmb_{}_mc_{:04}_005a_2048.fits'.format(subset, mc)
                fn_out = "{}/{:04}/smoothed_{:04}_{:03}{}.fits".format(
                    mapcache, mc, mc, freq, subset
                )
                if os.path.isfile(fn_in):
                    cleaned = load_map(fn_in, fn_out, nside, fwhm_rad, lmax) * 1e-6
                    cleaned[0] = dipo
    ...
    if do_plot:
        for imode, mode in enumerate(["EE", "BB", "TE"]):
            ...
            plt.legend(loc="best")
            plt.savefig("{}_bias_{:02}fsky{}.commander.png".format(mode, fsky, subset))
            # plt.show()
    ...
    fname_bl_in = (
        ...
        "Bl_TEB_npipe6v20_{:03}{}x{:03}{}.fits".format(
            subset, freqref, sname, freqref, sname
        )
    )
