...
if len(sys.argv) > 1:
    ...
    freqs = (100,)  # 30, 44, 70, 100, 143, 217, 353
norm = 1e15
...
ffpver = "ffp10_cmb"
mcstart, mcstop = 200, 800
#mcstart, mcstop = 200, 300
ver = "npipe6v20"
# mcstart, mcstop = 900, 902  # Test TFMODE

fsky = 80  # 60 # 27, 33, 39, 46, 53, 60, 67, 74, 82, 90
lmin_fit = 2
lmax_fit = 20
nside_fit = 16 # 32
...
#        --cmb_alm cmb_cache/ffp10_cmb_${freq}_alm_mc_{:04}.fits \
#        --quickpolbeam /global/cscratch1/sd/keskital/npipe_maps/npipe6v20/quickpol/Bl_TEB_npipe6v20_${freq}GHzx${freq}GHz.fits \


def get_mask(fsky, nside):
    fn_mask = "clmask_{:02}fsky_nside{:04}.fits".format(fsky, 256)
    mask = hp.read_map(fn_mask, verbose=False) > 0.5
...
for subset in "", "A", "B":
    ...
    for freq in freqs:
        ...
        # Load the Beam
        print("Loading", fn_beam_in, flush=True)
        ...
        for mc in range(mcstart, mcstop):
            ...
            if not os.path.isfile(fn_alm_cmb):
                ...
                print("Reading", fn_alm_cmb_in, flush=True)
                ...
                print("Writing", fn_alm_cmb, flush=True)
            ...
            if not os.path.isfile(fn_clean):
                print(fn_clean, "does not exist", flush=True)
                continue
        ...
        fit_maps = []
        fit_alms = []
        for m, alm in zip(maps, alms):
            alm2 = alm.copy()
            alm2[ind] *= 0
            cmb = hp.alm2map(alm2, nside, verbose=False, pixwin=True)
            fit_maps.append(hp.ud_grade(m - cmb, nside_fit))
            alm2 = alm.copy()
            alm2[indinv] = 0
        ...
        nparam = nreal + nimag
        print("There are {} TF params".format(nparam), flush=True)
        ...
        def get_resid(x):
            ...
            # print('Evaluating resid, x =', x, flush=True)
            tf = get_tf(x)
            nmap = len(fit_maps)
            ngood = np.sum(mask)
            resid = np.zeros(nmap * ngood * 2)
            nside = hp.get_nside(fit_maps[0])
            offset = 0
            for m, alm in zip(fit_maps, fit_alms):
                alm2 = alm.copy()
                alm2[ind] *= tf
                cmb = hp.alm2map(alm2, nside, verbose=False, pixwin=True)
                for i in [1, 2]:
                    resid[offset : offset + ngood] = (m[i] - cmb[i])[mask]
                    offset += ngood
            return resid
        ...
        def get_resid2(x):
            ...
            for m, alm in zip(fit_maps, fit_alms):
                ...
                cmb = hp.alm2map(alm2, nside, verbose=False, pixwin=True)
                for i in [1, 2]:
                    resid += np.sum((m[i] - cmb[i])[mask] ** 2)
            return resid

        print("Minimizing least squares", flush=True)
        if False:
            # TRF minimization with bounds
            chisq0 = get_resid(x0)
            ...
            if result.success:
                ...
                print(
                    'Minimization done: "{}". chisq0 = {}, chisq0 - chisq = {}, '
                    'nfev = {}'.format(
                        result.message, chisq0, chisq0 - chisq, result.nfev
                    ), flush=True
                )
            # Error estimates
            jac = result.jac
            ...
            # LM minimization without bounds uses less memory and is more
            # robust
            chisq0 = get_resid(x0)
            chisq0 = np.dot(chisq0, chisq0)
            result = least_squares(
            ...
            err = sigma * np.sqrt(np.diag(invhess))
            """
            chisq0 = get_resid2(x0)
            result = minimize(get_resid2, x0, tol=1e-15,
                              bounds=[[-10, 10]] * (nreal + nimag))
            if result.success:
                x = result.x
                chisq = get_resid2(x)
                print(
        ...
        for comp, fit in enumerate([fit_t, fit_e, fit_b]):
            ...
            ax.set_ylim([-0.5, 1.5])
        plt.savefig("anisotropic_tf_{:03}{}_fsky{:02}.png".format(freq, sname, fsky))

        tf = get_tf(result.x, full=True)
        tferr = get_tf(err, full=True)
        tf = np.vstack([tf, tferr])
        fnout = "anisotropic_tf_{:03}{}_fsky{:02}.fits".format(freq, sname, fsky)
