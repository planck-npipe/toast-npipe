...
def set_estimator(cross, freq1, freq2):
    ...
    if last_freqs[0] == freq1 and last_freqs[1] == freq2:
        return
    print("Constructing estimator for {}A x {}B".format(freq1, freq2), flush=True)
    deltaN = 1e-16
    dN = np.identity(cross.npix * cross.nstokes) * deltaN
    N1 = npipeqml.readcov(
        "/project/projectdirs/planck/data/npipe/npipe6v20A/lowres/"
        "npipe6v20A_ncm_ns0016_smoothed_{:03}_bin.dat".format(freq1),
        mask,
    )
    N1 += dN
    ...
    if freq1 != freq2:
        N2 = npipeqml.readcov(
            "/project/projectdirs/planck/data/npipe/npipe6v20B/lowres/"
            "npipe6v20B_ncm_ns0016_smoothed_{:03}_bin.dat".format(freq2),
            mask,
        )
        N2 += dN
    else:
...
for freqpair in freqpairs:
    ...
    #for mc in range(700, 800):
        fn_cl_clean = "{}/{:04}/clqml_{:04}_{:03}A_x_{:03}B_cleaned_{:02}fsky.fits".format(
        ...
        print("Loading", fn_cl_cmb, flush=True)
        cl_cmb = hp.read_cl(fn_cl_cmb)  # EE and BB

        print("Loading", fn_cl_clean, flush=True)
    ...
    for imode, mode in enumerate(["EE", "BB"]):
        ...
        for ell in range(2, nell + 2):
            ...
            if (ell - 2) // ncol == nrow - 1:
                ax.set_xlabel(r"Input C$_\ell$ x 1e15")
            if (ell - 2) % ncol == 0:
                ax.set_ylabel(r"Output C$_\ell$ x 1e15")

            plt.plot(xx, c * xx, label=r"k = {:.3f} $\pm$ {:.3f}".format(c, cerr))
            plt.legend(loc="best")
...
for imode, mode in enumerate(["EE", "BB"]):
    ...
    if do_plot:
        ...
        plt.savefig("{}_bias_{:02}fsky_AxB.png".format(mode, fsky))
        plt.show()
...
for freqpair in freqpairs:
    ...
    if freq1 == 0 and freq2 == 0:
        fname_bl_in = (
            ...
            "quickpol/"
            "Bl_TEB_npipe6v20_{:03}Ax{:03}B.fits".format(143, 143)
        )
    else:
        fname_bl_in = (
            "/project/projectdirs/planck/data/npipe/npipe6v20/"
            "quickpol/"
            "Bl_TEB_npipe6v20_{:03}Ax{:03}B.fits".format(freq1, freq2)
        )
    ...
    for w in [False]:
        if w:
            fname_bl = "Bl_TEB_xQML_npipe6v20_{:03}Ax{:03}B_with_E_tf.fits".format(
                freq1, freq2
            )
        else:
            fname_bl = "Bl_TEB_xQML_npipe6v20_{:03}Ax{:03}B_only_E_tf.fits".format(
                freq1, freq2
            )
        ...
        print("Wrote", fname_bl, flush=True)
