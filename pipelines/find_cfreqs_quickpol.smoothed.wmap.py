...
#datadir3 = "/Users/reijo/data/hfi_pipe/commander_2018/compsep_npipe5v21/"
...
for freq in sys.argv[1:]:
    try:
        ...
        # Non-integer argument, assume WMAP
        wmap = True
        band = freq
        freq = {"K" : 23, "Ka" : 33, "Q" : 41, "V" : 61, "W" : 94}[band]
        planckband = {"K" : "030", "Ka" : "030", "Q" : "044", "V" : "070", "W" : "100"}[band]
        flight = hp.read_map(f"{auxdir}/wmap/wmap_band_iqusmap_r9_9yr_{band}_v5.fits") * 1e-3
        fwhm = {"K" : 52.8, "Ka" : 39.6, "Q" : 30.6, "V" : 21.0, "W" : 13.2}[band]
        quickpol = None
    ...
    mask = (
        hp.ud_grade(
            hp.read_map(f"{auxdir}/bandpass_mask_{planckband}.fits", nest=True),
            nside,
            order_in="nest",
            order_out="ring",
        )
    ...
    sync = skymodel.eval(cfreq, synchrotron=True, freefree=False, ame=False, dust=False) * gain
    dust = skymodel.eval(cfreq, synchrotron=False, freefree=False, ame=False, dust=True) * gain
    ...
    if rank == 0:
        ...
        sync = hp.reorder(sync, r2n=True)
        dust = hp.reorder(dust, r2n=True)
        ...
        if wmap:
            ...
            fname_out_sync = (
                f"synchrotron_model_{band}_nside{nside:04}_cfreq_smoothed.fits"
            )
            fname_out_dust = (
                f"dust_model_{band}_nside{nside:04}_cfreq_smoothed.fits"
            )
            fname_out_deriv = (
            ...
            fname_out = (
                f"sky_model_{band}GHz_nside{nside:04}_quickpol_cfreq_zodi_smoothed.fits"
            )
            fname_out_sync = (
                f"synchrotron_model_{band}GHz_nside{nside:04}_quickpol_cfreq_zodi_smoothed.fits"
            )
            fname_out_dust = (
                f"dust_model_{band}GHz_nside{nside:04}_quickpol_cfreq_zodi_smoothed.fits"
            )
            fname_out_deriv = (
                f"sky_model_deriv_{band}GHz_nside{nside:04}_quickpol_cfreq_zodi_smoothed.fits"
            )
        hp.write_map(
            fname_out,
            model,
        ...
        print("Wrote", fname_out_sync, flush=True)
        hp.write_map(
            fname_out_dust,
            dust,
            nest=True,
        ...
        print("Wrote", fname_out_dust, flush=True)
        hp.write_map(
            fname_out_deriv,
            deriv,
            nest=True,
